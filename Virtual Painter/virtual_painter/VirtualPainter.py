...
import numpy as np #create canvas
import time 
import os #read file header
import HandTrackingModule as htm #read modul
...
myListDirectory = os.listdir("virtual_painter/header") #read file inside folder header
...
header = overlayList[0] #index 0 for overlayList as a default header
...
imgCanvas = np.zeros((720, 1280, 3), np.uint8) #create blank canvas

while True:
    res, frame = cap.read()
    frame = cv2.flip(frame, 1)  # flip frames
    frame = detector.findHands(frame) #called findHands() method for hand detection
    lmList = detector.findPosition(frame, draw=True) #put hand coordinate
    if len(lmList) != 0:
        x1, y1 = lmList[8][1:]
        x2, y2 = lmList[12][1:]
        print(x1, y1, x2, y2)
        ...
        if y1<125:
            if 320 < x1 < 480: #red brush
                header = overlayList[0]
                drawColor = (0,0,255)
            elif 480 < x1 < 630: #green brush
                header = overlayList[1]
                drawColor = (0, 255, 0)
            elif 630 < x1 < 840: #blue brush
    ...
    frame[0: 125, 0:1280] = header #show header
